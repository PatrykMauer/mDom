        ...
        if password==password2:
            ...
            if User.objects.filter(username=username).exists():
                ...
                if User.objects.filter(email=email).exists():
                    ...
                    user.save()
                    messages.success(request,'Rejestracja przebiegła pomyślnie. Prosimy o zalogowanie się')
...
def login(request):
    if request.method =='POST':
        ...
        if user is not None:
            ...
            messages.error(request,'Nieprawidłowe hasło lub login')
            return redirect('login')
    else:
...
def logout(request):
    if request.method=='POST':
        auth.logout(request)
        messages.success(request,'Jesteś wylogowany')
        return redirect('index')
